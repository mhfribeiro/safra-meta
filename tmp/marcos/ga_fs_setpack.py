...
class BaseProblem(object):
    ...
    def create_individual(self, _):
        ...
        # For the initial population, is convenient to represent the constraints by means of a matrix
        # It is better suitable for construction procedure
        local_constraints = np.zeros((len(self.attributes), len(self.attributes)), dtype=int)
        local_constraints_counts = np.zeros(len(self.attributes), dtype=int)
        for i in range(0, len(self.attributes)-1):
            for j in range(i+1, len(self.attributes)):
                if abs(self.corr[self.attributes[i]][self.attributes[j]]) >= self.corr_threshold:
                    local_constraints[i][j] += 1
                    local_constraints[j][i] += 1
                    local_constraints_counts[i] += 1
                    local_constraints_counts[j] += 1
        ...
        while count < n_attr:
            ...
            removed = [index]
            if local_constraints_counts[index]:
                for i, value in enumerate(local_constraints[index]):
    ...
    def precompute_violations(self, individual):
        ...
        if n_selected < self.min_size or n_selected > self.max_size:
            return len(self.constraints)+1, att_sel
        else:
            violations = 0
            for i in range(len(self.constraints)):
                if np.sum(np.bitwise_and(individual, self.constraints[i])) >= 2:
                    violations += 1
